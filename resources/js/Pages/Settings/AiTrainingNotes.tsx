...
interface Stats {
    ...
    rules: number;
    uses: number;
    topNote: { title: string; used: number } | null;
}
...
export default function AiTrainingNotes({ notes, pagination, filters, stats, graph }: AiTrainingNotesProps) {
    ...
    // ── List view: debounced semantic/keyword search + kind/status filters + pagination.
    const [listQuery, setListQuery] = useState(filters?.q ?? '');
    const [listKind, setListKind] = useState(filters?.kind ?? '');
    const [listStatus, setListStatus] = useState(filters?.status ?? '');
    const listQueryTimer = useRef<number | null>(null);

    const applyListFilters = (q: string, kind: string, status: string, page = 1) => {
        router.get(route('settings.ai.training-notes'), {
            q: q.trim() || undefined,
            kind: kind || undefined,
            ...
            preserveState: true,
            preserveScroll: true,
            only: ['notes', 'pagination', 'filters'],
        });
    };

    // Search is debounced so typing feels instant while the (semantic) lookup
    // only fires once you pause.
    useEffect(() => {
        if (listQueryTimer.current) window.clearTimeout(listQueryTimer.current);
        listQueryTimer.current = window.setTimeout(() => {
            if (listQuery.trim() !== (filters?.q ?? '')) {
                applyListFilters(listQuery, listKind, listStatus, 1);
            }
        }, 380);
        return () => {
            if (listQueryTimer.current) window.clearTimeout(listQueryTimer.current);
    ...
    const goPage = (page: number) => {
        ...
        applyListFilters(listQuery, listKind, listStatus, page);
    };

    // ── Mind-map detail + semantic memory search (on demand, keeps the payload light).
    const csrf = (document.querySelector('meta[name="csrf-token"]') as HTMLMetaElement)?.content || '';
    const jsonHeaders = { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': csrf };

    const loadNode = useCallback(async (id: number) => {
        try {
            const res = await fetch(route('settings.ai.training-notes.show', id), { headers: jsonHeaders });
            if (!res.ok) return null;
            const data = await res.json();
            return { content: String(data.note?.content ?? ''), links: Array.isArray(data.links) ? data.links : [] };
        } catch {
            return null;
        }
    }, []);
    ...
    const semanticSearch = useCallback(async (q: string) => {
        try {
            const res = await fetch(`${route('settings.ai.training-notes.search')}?q=${encodeURIComponent(q)}&limit=14`, { headers: jsonHeaders });
            if (!res.ok) return [];
            const data = await res.json();
                    ...
                    {view === 'list' && (
                        <div className="space-y-3">
                            ...
                            <div className="apple-card p-3 space-y-3">
                                <div className="flex items-center gap-2">
                                    <div className="relative flex-1">
                                        ...
                                        <input
                                            ...
                                            placeholder="Find a memory — semantic search (e.g. 'aturan garansi', 'keluarga', 'preferensi pelanggan')…"
                                            className="w-full rounded-xl border border-border/60 bg-background pl-9 pr-9 py-2 text-xs font-medium text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-primary"
                                        />
                                        {listQuery && (
                                            <button
                                                onClick={() => setListQuery('')}
                                                className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-full text-muted-foreground hover:text-foreground transition"
                                                title="Clear search"
                                            >
                                    ...
                                    <select
                                        ...
                                        onChange={e => {
                                            ...
                                            setListKind(k);
                                            applyListFilters(listQuery, k, listStatus, 1);
                                        }}
                                        className="rounded-xl border border-border/60 bg-background px-2.5 py-2 text-xs font-semibold text-foreground focus:outline-none focus:border-primary"
                                        title="Filter by brain kind"
                                    >
                                    ...
                                    <select
                                        ...
                                        onChange={e => {
                                            ...
                                            setListStatus(s);
                                            applyListFilters(listQuery, listKind, s, 1);
                                        }}
                                        className="rounded-xl border border-border/60 bg-background px-2.5 py-2 text-xs font-semibold text-foreground focus:outline-none focus:border-primary"
                                        title="Filter by active/paused"
                                    >
                                ...
                                <div className="flex items-center justify-between gap-2 flex-wrap">
                                    <p className="caption text-muted-foreground">
                                        Showing {safeNotes.length} of {pagination.total} nodes
                                        {filters.q ? (filters.method === 'semantic' ? ' — semantic match' : ' — keyword match') : ''}
                                    </p>
                                    {filters.q && (
                                        <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-semibold border ${filters.method === 'semantic' ? 'bg-primary/10 text-primary border-primary/20' : 'bg-muted/60 text-muted-foreground border-border/50'}`}>
                                            <Sparkles className="h-3 w-3" />
                                            {filters.method === 'semantic' ? 'Semantic search' : 'Keyword search'}
                                        </span>
                                    )}
                                </div>
                            </div>
                            ...
                            {safeNotes.length === 0 ? (
                                <div className="apple-card p-12 text-center text-muted-foreground space-y-3">
                                    ...
                                    <div className="space-y-1">
                                        {filters.q || filters.kind || filters.status ? (
                                            <>
                                                <p className="text1">No memory matches.</p>
                                                <p className="text2">Try a different keyword, or clear the search &amp; filters above.</p>
                                            </>
                                        ) : (
                                            <>
                                ...
                                {(pagination?.last_page ?? 1) > 1 && (
                                    <div className="apple-card p-2 flex items-center justify-between gap-2">
                                        <button
                                            onClick={() => goPage((pagination?.current_page ?? 1) - 1)}
                                            disabled={!pagination || pagination.current_page <= 1}
                                            className="inline-flex items-center gap-1 rounded-xl border border-border/60 bg-background px-3 py-1.5 text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-muted transition disabled:opacity-40 disabled:cursor-not-allowed"
                                        >
                                            <ChevronLeft className="h-3.5 w-3.5" />
                                            Previous
                                        </button>
                                        <span className="caption font-mono text-muted-foreground">
